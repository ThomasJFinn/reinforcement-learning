def multiarmed_bandit_eps_greedy_nonstat(num_bandits, q_mean, steps, eps, s):
    """
    Performs epsilon-greedy multi-armed bandit algorithm with epsilon decay to find the optimal bandit arm
    ...
    Parameters:
        ...
        eps_init (float): The initial exploration probability (epsilon) for selecting a random action
    ...
    for i in range(1, steps + 1):
        # Swap bandits with probability s
        p0 = random.uniform(0, 1)
        if p0 > s:
            pass
        else:
            q_mean = swap_bandits(q_mean)
            opt_bandit = np.argmax(q_mean)
        
        # Select next action using epsilon-greedy exploration with epsilon decay
